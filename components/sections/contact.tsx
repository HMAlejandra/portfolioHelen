...
export function Contact() {
  ...
  // Manejador de tema (Simulado para el Switcher visual)
  const toggleTheme = (selected: 'light' | 'dark') => setTheme(selected)
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-2 gap-24 mb-40">
          <div>
            ...
            <FadeIn delay={0.4}>
              <div className="border-l-2 border-[#ff4d00] pl-8 mb-12 opacity-70 italic text-xl">
            ...
            <FadeIn delay={0.6}>
              <Magnetic>
                <a href="mailto:helenmoncayo@gmail.com" className="group flex items-center gap-6">
                  ...
                  <span className="text-2xl md:text-3xl font-serif border-b border-transparent group-hover:border-[#ff4d00]">helenmoncayo@gmail.com</span>
                </a>
              </Magnetic>
            </FadeIn>
          </div>
        ...
        <div className="border-t border-current/10 pt-16 grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="flex flex-col gap-6">
            ...
            <p className="text-sm opacity-60 max-w-xs">I&apos;m Helen — a Software Engineer focused on web design and intelligent systems. Thanks for visiting!</p>
          </div>
          ...
          <div className="space-y-4 text-right md:text-left">
            <h5 className="font-mono text-[10px] font-bold tracking-widest opacity-40 uppercase">Connect</h5>
            <p className="text-sm opacity-60">Pasto, Nariño — Colombia</p>
            <p className="text-sm text-[#ff4d00] font-mono">UTC-5</p>
          </div>
        </div>

        {/* --- BARRA FINAL CON SWITCHER Y FLECHA HOME --- */}
        <div className="mt-20 pt-8 border-t border-current/5 flex flex-col md:flex-row justify-between items-center gap-8 pb-12">
          ...
          <div className="flex items-center gap-8">
            {/* SWITCHER LIGHT/DARK (Estilo Alessandro) */}
            <div className="flex items-center bg-current/5 p-1 rounded-full border border-current/10">
              <button 
                onClick={() => toggleTheme('light')}
                className={`p-2 rounded-full transition-all ${theme === 'light' ? 'bg-white text-black shadow-md' : 'opacity-40'}`}
              >
                <Sun className="w-3.5 h-3.5" />
              </button>
              <button 
                onClick={() => toggleTheme('dark')}
                className={`p-2 rounded-full transition-all ${theme === 'dark' ? 'bg-[#ff4d00] text-white shadow-md' : 'opacity-40'}`}
              >
                <Moon className="w-3.5 h-3.5" />
              </button>
            </div>

            {/* BOTÓN HOME (Flecha hacia arriba) */}
            <Magnetic>
              <button 
                onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
                className="w-12 h-12 rounded-full border border-current/20 flex items-center justify-center hover:bg-[#ff4d00] hover:border-[#ff4d00] hover:text-white transition-all group"
              >
                <ArrowUp className="w-5 h-5 group-hover:-translate-y-1 transition-transform" />
              </button>
            </Magnetic>

            {/* SOCIAL ICONS FINALES */}
